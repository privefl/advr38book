...
I recall that the `// [[Rcpp::export]]` makes the C++ function available to R, it doesn't export the function as part of your package (though you could access it with `<package>:::<rcpp-fun>()`). If you want to explicitly export an Rcpp function (as an R function), use roxygen comments (beginning with `//'` instead of `#'`) on top of your Rcpp function (and don't forget the `//' @export`).
