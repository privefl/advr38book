...
WORDS_TO_IGNORE <- c("truelength", "growable", "precomputing")
...
Hint: $\text{dist}(X_i, Y_j)^2 = (X_i - Y_j)^T (X_i - Y_j) = X_i^T X_i + Y_j^T Y_j - 2 X_i^T Y_j$.
...
Generate $10^7$ (start with $10^4$) steps of the process described by the formula:$$X(0)=0$$$$X(t+1)=X(t)+Y(t)$$ where $Y(t)$ are independent random variables with the distribution $N(0,1)$. Then, calculate in what percentage of indices $t$ the value of $X(t)$ was negative. You don't need to store values of $X$ if you don't want to.
...
Try to speed up this code by vectorizing it first, and by precomputing. Then, recode it in Rcpp and benchmark all the solutions you came up with.
