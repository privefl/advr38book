...
1. Make a function `mutate2` with the help of base R function `transform` (or `within`).

1. Bonus: make the previous functions more general by taking many arguments at once (in `...`). Make sure to keep your existing code as internal functions in order to break your code in manageable parts.

1. To use (and export) functions already implemented in other packages, for example the pipe from package {magrittr}, you can use `usethis::use_package("magrittr")` and put the following code somewhere in an R file of your package. Note that for this particular example, you could directly use `usethis::use_pipe()` only.
...
1. Implement more functions if you find this project interesting.
