...
1. Implement more functions if you find this project interesting. For example, make a function `mutate2` with the help of base R function `transform` (or `within`). All try to make the previous functions more general by taking many arguments at once (in `...`). **Make sure to keep your existing code as internal functions in order to break your code in manageable parts.**
...
To use (and export) functions already implemented in other packages, for example the pipe from package {magrittr}, you can use `usethis::use_package("magrittr")` and put the following code somewhere in an R file of your package. Note that for this particular example, you could directly use `usethis::use_pipe()` only.
...
```
In an R package, you don't need the first line but instead to add the package to the `LinkingTo` field of the *DESCRIPTION* file (e.g. with `usethis::use_package("RcppArmadillo", "LinkingTo")`).
...
- `inst/extdata/`: additional external data for examples and vignettes. See section \@ref(pkg-data) for more detail. You can also use `inst/testdata/` for data you need in tests. To access one file in this directory, use `system.file()`. For example, to find `inst/extdata/mydata.csv`, use `system.file("extdata", "mydata.csv", package = "mypackage")`.
...
- README + CRAN badge + URL/BugReports
- pkgdown
