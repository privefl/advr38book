...
```{r, include=FALSE}
WORDS_TO_IGNORE <- c("butitsuremakesthingseasiertoread", "addin", "Rproj", "Brun", "Julien", "CLI", "GPG")
...
```{r, error=TRUE, warning=TRUE, attr.source='.numberLines'}
for (x in 10:0) {
  y <- (x - 1)
  invisible(log(y))
}
...
In other situations, especially if you are new to `R`, the error (or warning) message may not help you at all. 
