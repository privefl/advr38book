...
Don't give up! In this section, we will look at some tools that you can use to find and fix problems.
...
Start by looking at the error (or warning) message; does it indicate what line or function causes the problem? In the example below, the warning message indicates that `log(y)` produced `NaNs`. As `log(y)` only occurs once, we know that the problem must be in line 
...
```
 ...
 A convenient solution to see all the variables' states in your code is to place some `browser()` from where you want to check the variables' states. To debug functions, `debugonce()` is also very useful.
