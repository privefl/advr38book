...
1. Use `replicate()` and `sample()` to get a 95% confidence interval for the mean of the following vector:
...
1. Use `match()` and some accessor to add a column "my_val" to this data `my_mtcars` by putting the corresponding value of the column specified in "my_col". [Can your solution be used for any number of column names?]
...
1. In the following data frame (recall that it is also a list), for the first 3 columns, replace letters by corresponding numbers based on the `code`:
