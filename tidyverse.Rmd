...
- Personnally, I also really like `data.table::read()` (with parameter `data.table = FALSE` if you want a `data.frame` instead of a `data.table`) which is very fast and convenient, and have been available for quite some time.
...
So, what does "tidy" means in "tidyverse"?
